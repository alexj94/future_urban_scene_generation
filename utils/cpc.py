...
# CamPoseCalib (CPC) method using Euler angles and translation vector
class CPC(torch.nn.Module):

    def apply_x_rot(self, alpha):
        cos = torch.cos(alpha)
        sin = torch.sin(alpha)
        self.Rx[1, 1], self.Rx[2, 2] = cos, cos
        self.Rx[1, 2], self.Rx[2, 1] = -sin, sin

    def apply_y_rot(self, alpha):
        cos = torch.cos(alpha)
        sin = torch.sin(alpha)
        self.Ry[0, 0], self.Ry[2, 2] = cos, cos
        self.Ry[0, 2], self.Ry[2, 0] = sin, -sin

    def apply_z_rot(self, alpha):
        cos = torch.cos(alpha)
        sin = torch.sin(alpha)
        self.Rz[0, 0], self.Rz[1, 1] = cos, cos
        self.Rz[0, 1], self.Rz[1, 0] = -sin, sin
    ...
    def __init__(self, focals: np.ndarray, centers: np.ndarray):
        ...
        self.register_parameter('Rx', torch.nn.Parameter(torch.eye(3)))
        self.register_parameter('Ry', torch.nn.Parameter(torch.eye(3)))
        self.register_parameter('Rz', torch.nn.Parameter(torch.eye(3)))
    ...
    def forward(self, points3d: torch.FloatTensor, points2d: torch.FloatTensor,
                initial_alphas: torch.FloatTensor, initial_tr: torch.FloatTensor,
                policy_iterations: callable, policy_lambda: callable):

        assert len(points3d) == len(points2d)
        assert len(initial_alphas) == len(initial_tr) == 3

        n_points = len(points3d)
        n_params = len(initial_alphas) + len(initial_tr)

        # graph inputs that need grad
        alpha_x = torch.nn.Parameter(initial_alphas[0])
        alpha_y = torch.nn.Parameter(initial_alphas[1])
        alpha_z = torch.nn.Parameter(initial_alphas[2])
        ...
        while policy_iterations(iteration, prev_error, cur_error, jac, error, updates,
                                [alpha_x.item(), alpha_y.item(), alpha_z.item(),
                                 tr_x.item(), tr_y.item(), tr_z.item()]):

            self.apply_x_rot(alpha_x)
            self.apply_y_rot(alpha_y)
            self.apply_z_rot(alpha_z)
            self.apply_tr(tr_x, tr_y, tr_z)

            points_3d_camera = self.Tr + (self.Rz @ self.Ry @ self.Rx @ points3d.T).T
            points_2d_pred = self.focals * points_3d_camera[:, :-1] / points_3d_camera[:, -1:] + self.centers

            error = points_2d_pred.detach() - points2d

            jac = self.compute_jacobian(jac, points_2d_pred, [alpha_x, alpha_y, alpha_z, tr_x, tr_y, tr_z])
            ...
            alpha_x = torch.nn.Parameter(alpha_x.data.detach() + updates[0])
            alpha_y = torch.nn.Parameter(alpha_y.data.detach() + updates[1])
            alpha_z = torch.nn.Parameter(alpha_z.data.detach() + updates[2])
        ...
        rt = torch.stack([alpha_x, alpha_y, alpha_z]).detach()
